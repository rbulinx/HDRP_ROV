...
{
    [Header("Source Cable")]
    public CableLMM_UnderwaterWinch_Collision cable;
    ...
    void Reset()
    {
        cable = GetComponent<CableLMM_UnderwaterWinch_Collision>();
    }

    void Start()
    {
        if (!cable) cable = GetComponent<CableLMM_UnderwaterWinch_Collision>();
    ...
    public void Rebuild()
    {
        ...
        for (int i = 0; i < segCount; i++)
        {
            ...
            {
                // LayerMaskから単一レイヤを想定：最下位bitのレイヤ番号を採用
