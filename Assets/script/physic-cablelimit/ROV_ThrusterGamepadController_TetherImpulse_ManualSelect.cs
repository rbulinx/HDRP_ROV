...
public class ROV_ThrusterGamepadController_TetherImpulse_ManualSelect : MonoBehaviour
{
    [Header("References (optional)")]
    public Transform mainCamera;

    [Header("Tether Cable (MUST assign in Inspector)")]
    [Tooltip("必ずInspectorでテザーケーブルを指定してください。未指定ならテザー拘束は一切かかりません。")]
    ...
    void Awake()
    {
        ...
        if (tetherCable == null)
        {
            Debug.LogWarning("[ROV_ThrusterGamepadController_TetherImpulse_ManualSelect] tetherCable が未設定です。テザー拘束は無効になります。");
        }
    }
